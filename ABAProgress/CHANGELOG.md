...
# 1.0.0 후보 빌드 16 — 시작 화면

- 앱을 열면 ‘쉬운 ABA’ 로고를 먼저 표시하고, 그 아래에 분홍색 이중 원형 파동 로딩 표시를 짧게 보여줍니다.
- 움직임 줄이기 설정에서는 파동을 정지 상태로 표시합니다. 출시 후보는 여전히 초안이며 배포하지 않았습니다.
...
- 실제 사용 흐름 영상은 11인치 iPad 시뮬레이터에서 기록하도록 고정했습니다.
...
- 홈 화면과 설치 이름을 ‘쉬운 ABA’로 통일하고 앱 진입 코드 이름을 `Easy_ABA`로 변경했습니다. 기존 Bundle ID를 유지해 저장된 기록과 설치 경로의 연속성을 보존합니다.
- 웹 보고서 편집을 기본 1시간으로 늘리고 30분 연장, 만료 전 저장 확인, 하단 타이머를 추가했습니다.
- 여러 프로그램의 보고서 정렬·접기/펼치기, PDF 레이아웃 및 기록 화면 이동을 개선했습니다.
- App Store 제출용 빌드 번호 15를 준비했습니다. 서명·TestFlight·실기기 검증·심사 제출은 운영 계정 설정 후 수행합니다.
