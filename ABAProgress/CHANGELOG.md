...
- Swift Playgrounds 패키지의 Xcode 소스 일치, 필수 리소스, 버전 및 Universal iOS 17 설정을 자동 검증하고 iPad용 다운로드 아티팩트를 생성합니다.
- 패키지 내부의 오래된 v0.6 실행 안내를 v0.12.0 설치·실행·데이터 보존 안내로 교체했습니다.
- 웹 편집 만료 시간 브라우저 검사는 실행 지연으로 1초가 지난 정상 상태도 허용하면서 약 60분 범위는 계속 검증합니다.
