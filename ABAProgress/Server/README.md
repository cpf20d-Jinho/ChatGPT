# ABAProgress 보고서 서버 (v0.10.0)

선택적 암호화 보고서 웹 편집은 `REPORT_WEB_EDITING.md`를 참고한다. 아래 숫자 전송 제한은 **AI 초안 API**에 적용된다. 별도 웹 편집 API는 사전 동의한 여섯 서술 항목의 암호문만 임시 중계하며 AI를 호출하지 않는다.

> v0.9 운영 연결은 `DEPLOYMENT.md`를 먼저 읽는다. 현재 운영 모드는 사용자별 SHA-256 토큰·만료일 비밀 파일을 요구하며 요청당 `X-ABA-Consent: numeric-v1`을 검사한다. 아래 공용 토큰 실행법은 로컬 개발 전용이다. 앱의 키 삭제는 Keychain에서 삭제하며 발급된 Groq 키 자체 폐기는 Console에서 별도로 한다. 앱 삭제 후 Keychain 자료가 반드시 삭제된다고 가정하지 않는다.
...
키를 앱 설정이나 보고서 데이터에 평문으로 저장하지 않는다. 기기 백업으로 이동하지 않는다. 앱의 키 삭제 기능을 이용해 기기에서 지우고 Groq Console에서 발급 키를 폐기할 수 있다. 재설치만으로 키가 지워진다고 가정하지 않는다.
