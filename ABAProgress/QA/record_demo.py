...
template=next(d for d in data['devices'][r] if d.get('isAvailable') and 'iPhone' in d['name'])
typeid=next(d['identifier'] for d in data['devicetypes'] if d['name']==template['name'])
device=run('xcrun','simctl','create','ABA Demo',typeid,r)
...
try:
 ...
 (out/'verified.txt').write_text('Real iOS simulator UI recording. Synthetic records. Manual narrative; no AI provider call. Ends at system PDF share sheet; no external submission.\n')
finally:
