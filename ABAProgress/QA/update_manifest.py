...
for name in ['BUILD_INFO.json', 'HANDOFF_MANIFEST.json']:
    ...
    if name == 'BUILD_INFO.json':
        data.update(version='0.10.0', buildTimeKST=now, buildHash=source[:10], sourceSHA256=source, previousBuild='v0.9.1_b9d0ded59d')
    else:
        data.update(sourceVersion='0.10.0', sourceBuild=now, sourceBuildHash=source[:10])
