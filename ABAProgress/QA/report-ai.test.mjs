...
test("criteria retained and unusual domain labels handled",()=>{const p=fixture();p.goals[0].criteria={"1":90};p.goals[0].domain="__proto__";assert.equal(validatePayload(p).goals[0].criteria["1"],90);assert.equal(validatePayload(p).domains[0].recentMean,40);p.goals[0].criteria["1"]=101;assert.throws(()=>validatePayload(p));});
