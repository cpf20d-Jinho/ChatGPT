...
enum ReportWebEditing {
    static let notice = """
선택한 보고서의 도전적 행동 변화·종합 현황·주요 변화·치료사 소견·가정 안내·다음 목표만 편집합니다. 아동 프로필·생년월일·시행 기록·그래프·표지·서명·참고 메모는 포함하지 않습니다. 문장에 직접 적은 개인정보는 자동으로 제거되지 않으므로 전송 전에 삭제하세요.

여섯 항목을 기기에서 암호화한 뒤 Render 보고서 서버(싱가포르)의 메모리에 최대 30분 임시 보관합니다. AI에는 보내지 않습니다. 무료 서버의 중지·재시작 시 더 일찍 사라질 수 있습니다. 서버 저장은 백업이 아니며, 앱 원본은 유지됩니다. 연결 과정에서 서비스 제공자가 IP 등 접속 정보를 처리할 수 있습니다.

전용 링크를 가진 사람은 해당 서술을 읽고 수정할 수 있습니다. 신뢰하는 기기에서만 열고 공유 대상에 주의하세요. 편집 후 앱에서 가져와 검토·반영해야 합니다. 앱을 종료하면 링크를 복구할 수 없으며 새 링크를 만들어야 합니다. 취소해도 기기 내 수동 작성은 계속 사용할 수 있습니다.
"""
